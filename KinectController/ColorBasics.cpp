...
void CColorBasics::ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight) 
{
    if (m_hWnd)
    {
        ...
        StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L" FPS = %0.2f    Time = %I64d", fps, (nTime - m_nStartTime)%100000000);
    ...
    if (pBuffer && (nWidth == cColorWidth) && (nHeight == cColorHeight))
    {
        ...
        if (m_bSaveScreenshot || (nTime - m_nStartTime) % 30000000 < 400000)
        {
			_cwprintf(L"%I64d\n", nTime - m_nStartTime);
...
HRESULT CColorBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        StringCchPrintfW(lpszFilePath, nFilePathSize, L"D:\\new\\%d.bmp", count++);
    }
...
HRESULT CColorBasics::SaveBitmapToFile(BYTE* pBitmapBits, LONG lWidth, LONG lHeight, WORD wBitsPerPixel, LPCWSTR lpszFilePath)
{
	...
	//transfer(pBitmapBits, dwByteCount);
	//transfer(t, 8);
    ...
    // Close the file
    CloseHandle(hFile);
	//savejpeg("ok.jpg", pBitmapBits, 1920, 1080, 3);
	/*FILE* fp;
	fp = fopen("D:\new\0.bmp","r");
	transfer(fp);
	fclose(fp);*/
	char a[100];
	sprintf(a, "py.py %d.bmp", count-1);
	system(a);
    return S_OK;
}